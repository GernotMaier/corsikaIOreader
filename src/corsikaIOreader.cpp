...
{
   vector< int > m( ntel, 0 );
   for( int i = 0; i < ntel; i++ ) m[i] = i;

   if( iCFGFile.size() == 0 )  return m;
   for( int i = 0; i < ntel; i++ ) m[i] = -1;
...
vector< double > getXYZlevels( double istart, double idiff )
{
    ...
    vector< double > iL;
    if( idiff <= 0. ) return iL;

    int nlevel = (int)((1005.-istart)/idiff);

    if( bDebug ) cout << "filling XYZ levels: " << endl;
    for( int i = 0; i <= nlevel; i++ )
    {
         ih = istart+i*idiff;
         iL.push_back( heighx_( &ih ) / 100. );
	 if( bDebug ) cout << "\t XYZ level " << i << "\t" << ih << " [g/cm2] " << iL.back() << " [m]" << endl;
    }
    return iL;
}
...
// global quantum efficiency should be between 0. and 1.
   ...
   if( !bstdout ) cout << "SEED (for Cherenkov photon wavelengths): " << (int)fRandom.GetSeed() << endl;
   if( !bstdout ) cout << "ntel mode " << nTel << endl;
...
//   iobuf->max_length = 10000000000;
   if( !bstdout ) printf( "Maximum buffer length: %ld\n", iobuf->max_length );
   if( !bstdout ) cout << "Photon output file " << fGrisuOutputFile << endl;

// try to open Corsika file 
   data_file = 0;
   if( !bstdout ) printf("Input file: %s\n", fCorsikaIO.c_str() );
	...
	//possible return values:  0 (O.k.), -1 (error), -2 (end-of-file), -3 (block skipped because it is too large).
	 ...
	 switch (i_block) 
	 {
      ...
      switch ( block_header.type )
      {
	    ...
	    if( bPrintHeaders ) cout << "IO_TYPE_MC_RUNH" << endl;
...
// observation level from Corsika 
	    if( array.obs_height > 0. )
	    {
	       ...
	       for( unsigned int p = 0; p < fGrisu.size(); p++ ) fGrisu[p]->setObservationHeight( array.obs_height * 0.01 );
            }
...
// fill corsika run header class
	 ...
	 case IO_TYPE_MC_INPUTCFG:
	    if( bPrintHeaders ) cout << "IO_TYPE_MC_INPUTCFG" << endl;
	    ...
	    if( bPrintHeaders ) cout << "IO_TYPE_MC_TELPOS" << endl;
...
// initialize grisu readers:
	        ...
	        if( nTel > -2 )
		{
		   ...
		   if( fGrisuOutputFile.size() > 0 ) fGrisu.back()->setOutputfile( fGrisuOutputFile );
                }
		else if( nTel == -2 )
		{
		   char hO[2000];
	    ...
	    if( bPrintHeaders ) cout << "IO_TYPE_MC_EVTH" << endl;
	    if( bDebug ) cout << "reading IO_TYPE_MC_EVTH" << endl;
...
// read atmospheric profile from CHERENKOV OPTIONS
 ...
 //(GM) keep Corsika coordinate system            az  = 180. - (180./M_PI)*(evth[11]-evth[92]);   
	    ...
	    if( bPrintHeaders ) cout << "IO_TYPE_MC_TELOFF" << endl;
	    ...
	    if( bDebug ) cout << "\t total number of arrays simulated: " << array.mc_run.num_arrays << endl;
            break;

         /* Photon data for a complete array (one of perhaps many instances) */
         case IO_TYPE_MC_TELARRAY:
	    if( bPrintHeaders ) cout << "IO_TYPE_MC_TELARRAY" << endl;
	    if( readNarray >= narray && narray >= 0 ) break;
	    ...
	    if( bDebug ) cout << "\t shower core for array " << readNarray << " at (x/m) [m]: " << array.shower_sim.xcore << "\t" << array.shower_sim.ycore << endl;
   ...
   /* Distances of telescopes from shower axis */
	    ...
	    if( bHisto ) fHisto->newEvent( evth, array, iarray );                      // start new event for each array
	    if( bGRISU )
	    {
	        for( unsigned int p = 0; p < fGrisu.size(); p++ ) fGrisu[p]->writeEvent( array, bPrintMoreInfo );
            }

            for (itc=0; itc<array.ntel; itc++)
            {
               sub_item_header.type = IO_TYPE_MC_PHOTONS;
               if ( search_sub_item(iobuf,&item_header,&sub_item_header) < 0 ) break;
	       ...
	       if( nTel >= 0 && itel != nTel ) continue;
// fill number of photons per telescope (ignore telescope matrix, this is for tcors!)
	       if( bHisto ) fHisto->fillNPhotons( itel, photons );

// check if this telescope should be analysed
	       if( itel < (int)fTelescopeMatrix.size() && fTelescopeMatrix[itel] < 0 ) continue;
...
// fill all bunch specific stuff into histograms
                  if( bHisto ) fHisto->fillBunch( bunches[ibunch], corstime ); 
// now loop over bunch
...
// fill generated photons into histograms
		     if( bHisto ) fHisto->fillGenerated( Chphoton, prob );
// extinction + efficiencies
                     if ( bunches[ibunch].photons < 1. ) prob *= bunches[ibunch].photons;
                     if ( prob <= 1. )
                     {
		         double iRand = fRandom.Uniform( 1. );
			 if ( iRand > prob ) continue;
     // fill histograms without quantum efficiency applied (only atm.extinction)
		         if( bHisto ) fHisto->fillSurvived( Chphoton, prob );
     ...
     // write photons to iotxt output file (after quantum efficiency)
			 if( bGRISU )
			 {
			    if( nTel > -2 )
			    {
			        if( fGrisu.size() == 1 ) fGrisu[0]->writePhotons( Chphoton, fTelescopeMatrix[itel] ); 
                            }
			    else if( nTel == -2 )
			    {
...
// telescope ID is always 0
				if( itel < (int)fGrisu.size() ) fGrisu[itel]->writePhotons( Chphoton, 0 );
                            }
                         }
                     }
		  }
	    ...
	    if( bPrintHeaders ) cout << "IO_TYPE_MC_EVTE" << endl;
	    ...
	    if( bPrintHeaders ) cout << "IO_TYPE_MC_RUNE" << endl;
            read_tel_block(iobuf,IO_TYPE_MC_RUNE,rune,273);
            break;

         /* Unknown / any other material */
         default:
	    if( bPrintHeaders)  cout << "Read block header of type " << block_header.type << ", will ignore." << endl;
            break;
   ...
   if( bHisto && fHisto ) fHisto->terminate();  
   if( !bstdout )
   {
      if( fRunHeader ) fRunHeader->printHeader( cout );
      cout << endl;
