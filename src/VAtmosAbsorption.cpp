...
// CORSIKA extinction values
   ...
   else if( fModel == "CORSIKA" || fModel == "corsika" )
   {
      ...
      if( iSourceFile.size() > 0 ) fSourceFile = iSourceFile;
      else                         fSourceFile = "data/atmabs.dat";
...
// kascade extinction values
   ...
   {
      if( iSourceFile.size() > 0 ) fSourceFile = iSourceFile;
      else                         fSourceFile = "data/kextint.dat";
...
// MODTRAN4 US76 extinction values
   else if( fModel == "us76_new" || fModel == "us76.50km" || fModel == "us76.50" || fModel == "modtran4" )
   {
      ...
      if( iSourceFile.size() > 0 ) fSourceFile = iSourceFile;
      else                         fSourceFile = "data/us76.50km.ext";
      read_extint( 200 );
   }
   else if( fModel == "us76.23km" || fModel == "us76.23" || fModel == "modtran4_2"  )
   {
      ...
      if( iSourceFile.size() > 0 ) fSourceFile = iSourceFile;
      else                         fSourceFile = "data/us76.23km.ext";
      read_extint_F2( 200 );
   }
   else if( fModel == "artemis" )
   {
      if( iSourceFile.size() > 0 ) fSourceFile = iSourceFile;
      else                         fSourceFile = "data/extinction_uv.dat";
      read_extint( 180 );
   }
...
// MODTRAN4 data is from 200nm only
	if( fModel == "modtran4" || fModel == "modtran4_2" ) minwave = 200;
	if( fModel == "modtran5" ) minwave = 205;
...
double VAtmosAbsorption::getWavelength( double emissionheigth, double emissionangle )
{
    ...
    if ( fRandom->Uniform( 1. ) > prob ) return -1;
...
//   cout << "VAtmosAbsorption: reading atmospheric extinction file (lambda > " << LAMBDA_MIN << " nm): " << fSourceFile << endl;
  ...
  if( finp != NULL )
  {
      ...
      {
	  /* Read in wavelength. */
	  if( fModel != "artemis" ) fscanf(finp,"%d %*[^\n]\n",&lambda_in);
	  else                      fscanf(finp,"%d \n",&lambda_in);
	  ...
	  {
	     if( fModel != "artemis" ) fscanf(finp," %lf %*c",&extint[i][bin]); 
// artemis file without ","
	     else fscanf(finp," %lf ",&extint[i][bin]); 
  ...
  /* Program to read in extinction coefficients. Very finely tuned to file
   ...
   while( getline( is, is_line ) )
   {
	 ...
	 for( int i = 0; i < ALT_STEPS; i++ )
	 {
	    ...
	    if( !isnormal( extint[i][bin] ) ) extint[i][bin] = 1.e10;
         }
      }
   }
}
